...
glove = Glove.load("./glove/pretrained_models/glove_500.model")
...
norms = {}
for idq in range(len(glove_query_vectors)):
  index = 2*idq+1
  norms[str(index)] = {}
  query = glove_query_vectors[idq]
...
pd.DataFrame(data=list(norms.values()),columns=norms["1"].keys(),index=list(norms.keys())).to_csv("./glove/preprocessing/cosine_similarity_matrix.csv")
cosine_similarity_matrix = pd.read_csv("./glove/preprocessing/cosine_similarity_matrix.csv", index_col=0)
...
outfile = open("./glove/results/myresults_cos_500.txt", "w")
outfile.write(results)
outfile.close()

myqrels = ""

labels = {int(k):v for k,v in labels.items() if int(k)%2==1}      # we are interested in even topics, but labels start from 1
...
outfile = open("./glove/results/myqrels_cos_500.txt", "w")
