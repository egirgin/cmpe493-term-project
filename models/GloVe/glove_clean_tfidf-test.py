...
sf = cvec.fit_transform([" ".join(doc) for doc in lst_words])

transformer = TfidfTransformer()
transformed_weights = transformer.fit_transform(sf)
weights = np.asarray(transformed_weights.mean(axis=0)).ravel().tolist()
weights_df = pd.DataFrame({'term': cvec.get_feature_names(), 'weight': weights})
weights_df = weights_df.set_index('term').T
...
for idx in range(len(lst_index)):
  ...
  for word in lst_words[idx]:
    try:
      word_vectors.append(glove.word_vectors[glove.dictionary[word]] * weights_df[word]["weight"])
    except: ""
  try:
...
for idx in range(len(query_ids)):
  ...
  for word in query_texts[idx]:
    query_vectors.append(glove.word_vectors[glove.dictionary[word]] * weights_df[word]["weight"])
  try:
...
pd.DataFrame(data=list(norms.values()),columns=norms["2"].keys(),index=list(norms.keys())).to_csv("./models/GloVe/glove/preprocessing/cosine_similarity_matrix_tfidf_test.csv")
cosine_similarity_matrix = pd.read_csv("./models/GloVe/glove/preprocessing/cosine_similarity_matrix_tfidf_test.csv", index_col=0)
results = ""
...
outfile = open("./models/GloVe/glove/results/myresults_tfidf_cos_500_test.txt", "w")
...
outfile = open("./models/GloVe/glove/results/myqrels_tfidf_cos_500_test.txt", "w")
